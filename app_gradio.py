...
model = YOLO('./models/FastSAM-x.pt')
...
def segment_everything(
    ...
    new_w = int(w * scale)
    new_h = int(h * scale)
    _input = _input.resize((new_w, new_h))

    results = model(_input,
                    device=device,
                    retina_masks=True,
                    iou=_iou_threshold,
    ...
    if len(text) > 0:
        ...
        annotations, _ = text_prompt(results, text, _input, device=device, wider=wider)
